...
const DocumentsPage: React.FC = () => {
    ...
    const fetchDocuments = async () => {
      try {
        ...
        if (userRole === 'admin' || userRole === 'user') {
          response = await axios.get('https://bnr-archive-management-system.onrender.com/document/documents', {
        ...
        notification.error({ message: 'Failed to fetch the documents' });
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const handleSearch = async () => {
      if (searchTerm.trim() === '') {
        try {
          const response = await axios.get('https://bnr-archive-management-system.onrender.com/document/documents', {
            headers: { Authorization: `Bearer ${token}` }
          });
          setDocuments(response.data);
  ...
  return (
    <div>
      ...
      <div className="search-bar "  style={{    borderColor: '#800000', color: '#FFFFFF',width:200 }}>
        <Input
          type="text"
