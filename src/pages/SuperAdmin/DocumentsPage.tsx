...
import '../../styles/AdminStyles/DocumentsPage.css';
...
const SuperAdminDocumentsPage: React.FC = () => {
    ...
    const fetchDocuments = async () => {
      try {
        ...
        if (userRole === 'admin' || userRole === 'user') {
          response = await axios.get('http://localhost:8000/document/documents', {
  ...
  const handleSearch = async () => {
    if (searchTerm.trim() === '') {
      try {
        const response = await axios.get('http://localhost:8000/document/documents', {
          headers: { Authorization: `Bearer ${token}` }
        });
        setDocuments(response.data);
...
useEffect(() => {
  ...
  return (
    <div>
         <div className="search-add-container2">
